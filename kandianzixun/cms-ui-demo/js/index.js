$(function () {
  $('body>nav').on('click', '.list>ul ul>li', function (e) {
    ...
    e.stopPropagation();
    if (text === "用户管理") {
      $('.content .container').load('./pages/userManage.html');
      return;
    }
    if (text === "权限管理") {
      $('.content .container').load('./pages/privilegeManage.html');
      return;
    }
    if (text === "角色管理") {
      $('.content .container').load('./pages/roleManage.html');
      return;
    }
  })
  $('body>nav').on('click', '.list>ul>li', function (e) {
    ...
    if (text === "分类管理") {
      $('.content .container').load('./pages/categoryManage.html');
      return;
    }
    if (text === "资讯管理") {
      $('.content .container').load('./pages/articleManage.html');
      return;
    }
    if (text === "评论管理") {
      $('.content .container').load('./pages/commentManage.html');
      return;
    }
    if (text === "个人信息") {
      $('.content .container').load('./pages/userInfo.html');
      return;
    }
  })
  // 左侧菜单控制
